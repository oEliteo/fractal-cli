...
int main() {
  ...
  while (1) {
    if (read(STDIN_FILENO, &input_char, 1) == 1) {
      ...
      if (input_char == 'i') {
        // Zoom real axis
        long double zoom_real = (ctx->real_max - ctx->real_min) * 0.9;
        long double zoom_imag = (ctx->imaginary_max - ctx->imaginary_min) * 0.9;
        const long double smallest = 1e-17L;
        if (zoom_real < smallest || zoom_imag < smallest) {
          changed = 0;
        } else {
          center = (ctx->real_max + ctx->real_min) / 2.0;
          ctx->real_max = center + (zoom_real / 2.0);
          ctx->real_min = center - (zoom_real / 2.0);
          // Zoom Imaginary axis
          center = (ctx->imaginary_max + ctx->imaginary_min) / 2.0;
          ctx->imaginary_max = center + (zoom_imag / 2.0);
          ctx->imaginary_min = center - (zoom_imag / 2.0);
          changed = 1;
        }
      }
      ...
      if (input_char == 'o') {
        // Zoom real axis
        long double zoom_real = (ctx->real_max - ctx->real_min) * 1.1;
        long double zoom_imag = (ctx->imaginary_max - ctx->imaginary_min) * 1.1;
        const long double smallest = 1e-17L;
        if (zoom_real < smallest || zoom_imag < smallest) {
          changed = 0;
        } else {
